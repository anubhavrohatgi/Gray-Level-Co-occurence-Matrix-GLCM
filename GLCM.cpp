...
void GLCM::setImage(cv::Mat& input)
{ 
...
void GLCM::splitToWindows(int win_size) //size should be a square size
{
	...
	cv::Mat glBlank = cv::Mat::zeros(image_levels,image_levels,CV_64FC1);
...
void GLCM::computeGLCMParams()
{
	...
	for(size_t k = bl.begin(); k!=bl.end();k++)
	{
		...
		_glcmBlocks[k].glcm_avg = (_glcmBlocks[k].glcm_0 + _glcmBlocks[k].glcm_45 + _glcmBlocks[k].glcm_90 + _glcmBlocks[k].glcm_135) / 4.0;
